...
export const deleteAttendance = async (req, res) => {
  try {
    ...
    if (!memberId || !date) {
      return res.status(400).json({ message: 'memberId et date sont requis' });
    }
    ...
    if (!deleted) {
      return res.status(404).json({ message: 'Aucune présence trouvée pour ce membre à cette date' });
    }

    res.json({ message: 'Présence annulée avec succès', memberId });
  } catch (error) {
