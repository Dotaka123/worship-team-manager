...
// ✅ NOUVEAU: Supprimer une présence par memberId et date
export const deleteAttendanceByMemberAndDate = async (req, res) => {
  try {
    const { memberId, date } = req.body;

    if (!memberId || !date) {
      return res.status(400).json({ 
        message: 'memberId et date sont requis' 
      });
    }
    ...
    res.json({ message: 'Présence annulée avec succès' });
  } catch (error) {
    console.error('❌ deleteAttendanceByMemberAndDate error:', error);
    res.status(500).json({ message: error.message });
  }
};
